...
from serial_port import openPort, writePort, readPort
import numpy as np
import time
import cv2
...
classify = {"1": ["bottle", "can"], "2": ["bricks"], "3": ["battery", "cigarette"],
            "4": ["vegetable", "fruit"]}
...
class cameraThread(QObject):
    ...
    def run(self):
        ...
        while True:
            # check serial port
            if readPort() == "ok":
                sort_active = bool(1 - sort_active)
            ...
            if not sort_active:
                ...
                net.setInput(blob)
                start = time.time()
                layerOutputs = net.forward(ln)  # predict
                end = time.time()
                ...
                if len(idxs) > 0:
                    ...
                    for i in idxs.flatten():
                        ...
                        # writePort(ser, labels[classIDs[i]])
                        for ids in classify:
                            print("get in")
                            if labels[classIDs[i]] in classify[ids]:
                                writePort(ser, str(ids))
                                # sort_active = bool(1 - sort_active)
                                print("serial transmit = " + str(ids))
                ...
                if len(value) != 0:
                    ...
                    # if not sort_active:
                    #     self.items.emit(garbage)   # send signals
                    self.img.emit(frame)
                    # time.sleep(2)  # delay 2s
                    # sumit single to stm32
                if not sort_active:
                    self.items.emit(garbage)  # send signals
                    self.img.emit(frame)
